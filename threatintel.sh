#!/bin/bash
API_KEY="your_api_key_here" # Replace with VirusTotal or AbuseIPDB API Key
TARGET_FILE=$1
OUTPUT_DIR="reports"
mkdir -p $OUTPUT_DIR
...
curl -s -X GET "https://www.virustotal.com/api/v3/ip_addresses/$1" \
-H "x-apikey: $API_KEY" | jq '.' > "$OUTPUT_DIR/$1-threatintel.json"
}
function nmap_scan() {
...
echo "[+] Report for $target generated!"
echo ""
done < $TARGET_FILE
